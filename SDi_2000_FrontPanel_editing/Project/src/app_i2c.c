...
enum {
    ...
    STM8_LEFT_ADDR				= 0x50,//π¶¬ ƒ£øÈ0
    STM8_RIGHT_ADDR				= 0x52,//π¶¬ ƒ£øÈ1
    NJU72342_ADDR				= 0x80,//NJU72342µÁ◊”“Ù¡ø–æ∆¨
    OLED_ADDR					= 0x78, //OLEDœ‘ æ∆¡
    PCF8574_ADDR				= 0x40 //PCF8574“Ù∆µ—°‘Ò–æ∆¨, ‰»Î‘¥£¨ ‰≥ˆƒ£ Ω
} i2c_addr_TypeDef;
...
void Screen_Init(Screen_InitTypeDef *hScreen)
{
	...
	OLED_SetContrastControl(0xff);						//∂‘±»∂»0x80
	...
	//Power up VCC & Stabilized (Delay Recommended)  //Power up VCC≤ª»∑∂®»Á∫Œ≤Ÿ◊˜
	DelayNopMs(1);
...
void Screen_SetCoordinate(uint8_t x_point, uint8_t y_point)
{
  ...
  OLED_SetColumnAddress4LowerBits(x_point + 2);//Œ™∫Œ–Ë“™º”2
	OLED_SetColumnAddress4HigherBits(x_point + 2);
}

//ÊòæÁ§∫Â±èÊòØ8Ë°åÔºåËÄåËèúÂçïÊòØ4Ë°åÔºåÊØèË°åËèúÂçïÂç†Áî®‰∏§Ë°åÊòæÁ§∫Â±è ÊâÄ‰ª•‰º†ÈÄíÊó∂ÂÄôyË°åÂè∑ÈÉΩË¶Å‰πò‰ª•2
...
extern const unsigned char UnitSpace_12x32[][16]; //ø’∏Ò
extern const unsigned char SelFlag_8x16[][16];//—°‘Ò±Í÷æ
...
///OLED--Ê†πÊú¨ËÆæÂ§á        Screen--Â±ûÊÄß    Display--ÊòæÁ§∫ÁïåÈù¢
...
//void Display_ShowString(uint8_t horizontal_start, uint8_t line, char *str, bool reverse)  //reverse ”¶∏√ «∆Ï±Í◊¥Ã¨
...
void Display_Show12x32BigString(Display_HandlerTypeDef *hDisplay)
{
    ...
    unsigned char xIndex = hDisplay->HorizontalStart + 4; //∂‡≥ˆ8∏ˆ∑÷±Ê¬  Œ™¡ÀÕ∑Œ≤∂‘≥∆ Õ∑ø’Àƒ∏ˆ
    unsigned char yIndex = hDisplay->Line * 2;//∂ØÃ¨––∫≈
    unsigned char i = 0;
    unsigned char *atari8x16Char;

	Screen_DrawAreaToWhiteOrBlack(0, yIndex, 4, 4, FALSE);//«Â¿Ì«∞√ÊÀƒ∏ˆœÒÀÿ
...
-
*****************************************/
#define AT24C02_WRITE_PAGE_SIZE 8	//“∂øÌ
...
uint8_t write_eeprom(uint16_t start_addr, uint8_t *buf, uint16_t length)
{
	...
	full_page_cnt = (length - first_page_size)/AT24C02_WRITE_PAGE_SIZE;			//¬˙“≥ ˝¡ø
...
*
*****************************************/
#define I2C_SPEED             	 100000		//400000ÀŸ¬ œ¬£¨∂¡»°EEPROMª·”– ˝æ›¥ÌŒÛ
...
uint8_t I2C_BSP_Read(uint8_t slave_addr, uint8_t reg_addr, uint8_t *data, uint16_t length)
{
	...
	I2C_AcknowledgeConfig(I2C_ACK_CURR);			//±ÿ–Îœ» πƒ‹“ª¥ŒACK
...
{
	//I2C_AcknowledgeConfig(I2C_ACK_CURR);// «∑Ò–Ë“™
		...
		while(length > 0)
		{
			...
			while(!I2C_CheckEvent( I2C_EVENT_MASTER_BYTE_TRANSMITTED))	 //ø…øø,ÀŸ∂»±‰¬˝EV8_2
			{
				if((I2CTimeout--) == 0)
					return i2c_error_callback();
			}

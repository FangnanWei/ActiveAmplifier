...
{
	volatile uint8_t isHighPinMask = (GPIOx->IDR & (uint8_t)GPIO_Pin);
	return isHighPinMask & GPIO_Pin ? TRUE : FALSE;
//  return ((BitStatus)(GPIOx->IDR & (uint8_t)GPIO_Pin));
}

uint8_t GPIO_ReadInputArray(GPIO_TypeDef* GPIOx, GPIO_Pin_TypeDef GPIO_PinMask) {
	return (GPIOx->IDR & (uint8_t)GPIO_PinMask);
}

...
extern CLK_HandlerTypeDef hCLK;
...
/**
  ...
  * @retval None
  */
extern bool clk_switch_success;
INTERRUPT_HANDLER(CLK_IRQHandler, 2)
{
	hCLK.SwitchSuccess = TRUE;
...
/**
  ...
  * @retval None
  */
extern IWDG_InitTypeDef hIWDG;
extern uint32_t g_time_cnt;
uint32_t IWDG_FeedTimeCnt = 0;
extern struct device_global_prmt g_dev;
extern Encoder_HandlerTypeDef hEncoder;
 INTERRUPT_HANDLER(TIM3_UPD_OVF_BRK_IRQHandler, 15)
 {
	...
	{
		Encoder_Scan(&hEncoder);
	}

	if (((g_time_cnt - IWDG_FeedTimeCnt) > 500) && (hIWDG.Inited != FALSE))
	{
		IWDG_Feed();
	}
 }
...
{
	if (I2C_GetFlagStatus(I2C_FLAG_OVERRUNUNDERRUN) != RESET) {
		I2C_ClearFlag(I2C_FLAG_OVERRUNUNDERRUN);

	}
	if (I2C_GetFlagStatus(I2C_FLAG_ACKNOWLEDGEFAILURE) != RESET) {
		I2C_ClearFlag(I2C_FLAG_ACKNOWLEDGEFAILURE);

	}
	if (I2C_GetFlagStatus(I2C_FLAG_ARBITRATIONLOSS) != RESET) {
		I2C_ClearFlag(I2C_FLAG_ARBITRATIONLOSS);

	}
	if (I2C_GetFlagStatus(I2C_FLAG_BUSERROR) != RESET) {

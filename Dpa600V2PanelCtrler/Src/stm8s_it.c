...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
volatile unsigned long g_TimeCnt = 0;
...
//u32 FifoWriteErrCnt = 0;
 INTERRUPT_HANDLER(UART2_RX_IRQHandler, 21)
 {
    ...
    */
    //disableInterrupts();
    while (UART2_GetFlagStatus(UART2_FLAG_RXNE)) {
	    char temp = UART2_ReceiveData8();
		if (!FifoWrite(&g_UartFifo, &temp, 1)) {
			//FifoWriteErrCnt++;
		}
    }
	//enableInterrupts();

...
{   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,// sp
    ...
    0x00, 0x55, 0x2A, 0x55, 0x2A, 0x55, 0x00, 0x00,// 55
    ...
    0x14, 0x14, 0x14, 0x14, 0x14, 0x14, 0x00, 0x00// horiz lines
};
...
const uint8_t F8X16[]=	  
{
    ...
    0x00,0xF8,0xF8,0xF8,0x00,0x00,0x00,0x00,0x00,0x0F,0x0F,0x0F,0x00,0x00,0x00,0x00,/*"$--闊抽噺",4*/
    0x18,0x3C,0x24,0xBC,0xD8,0x60,0x30,0x00,0x00,0x06,0x03,0x0D,0x1E,0x12,0x1E,0x0C,/*"%",5*/
    0x00,0xF8,0xF8,0xF8,0x00,0xF8,0xF8,0xF8,0x00,0x0F,0x0F,0x0F,0x00,0x0F,0x0F,0x0F,/*"&--闊抽噺",6*/
    ...
    0x00,0x00,0xC0,0xE0,0xF0,0xF8,0x00,0x00,0x00,0x00,0x00,0x01,0x03,0x07,0x00,0x00,/*"fg",95*/
#endif
};
...
bool Oled::Is6_8Chr(uint8_t chr) {
...
//mode:0,反白显示;1,正常显示				 
//size:选择字体 16/12 
void Oled::ShowChar(uint8_t x, uint8_t y, uint8_t chr, bool in8by16) {
	uint8_t c=0,i=0;	
	c=chr-' ';//得到偏移后的值			
    ...
    if(in8by16)
	{
        ...
        for(i=0;i<8;i++)
   		...
   		SetPos(x,y);
		for(i=0;i<4;i++)
			WriteOneByte(oledOp_Data, F4x8[c][i]); 
   		SetPos(x,y+1);
		for(i=0;i<4;i++)
			WriteOneByte(oledOp_Data, F4x8[c][i+4]); 
    }
}
...
void Oled::ShowNum(uint8_t x, uint8_t y,uint32_t num, uint8_t len, uint8_t size2) {
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				ShowChar(x+(size2/2)*t,y,' ', true);
				continue;
			}else enshow=1; 
		 	 
		}
	 	ShowChar(x+(size2/2)*t,y,temp+'0', true); 
	}
}
...
void Oled::ShowString(uint8_t x, uint8_t y, const uint8_t *chr) {
	...
	while (chr[j]!='\0')
	{
		is6_8 = Is6_8Chr(chr[j]);
		ShowChar(x,y,chr[j], !is6_8);
		x+= is6_8? 4 : 8;
		if(x>120){x=0;y+=2;}
		j++;
	}
...
void Oled::ClearArea(uint8_t xStart, uint8_t yStart, uint8_t xSize, uint8_t ySize)
{
    uint8_t x, y;
    for (y = 0; y < ySize; y++) {
		SetPos(xStart, yStart + y);
        for (x = 0; x < xSize; x++) {
			WriteOneByte(oledOp_Data, 0x00);
        }
    }
}

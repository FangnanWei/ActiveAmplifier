...
const char SoftVersion[] __at(USER_FLASH_FIRST_PAGE_ADDRESS+1024) = {" 1.0.1 Build @ "__DATE__" "__TIME__};
...
int main(void)
{
	...
	while(1)
	{
		...
		if (Time_GetSysMs() - adcTime >= ADC_SCAN_Time)
		{
			adcTime = Time_GetSysMs();
			Adc_Deal(inst);
		}

...
static int I2c_Read(const struct I2c *self, unsigned char slaveAddr, unsigned char addr, unsigned char *buf, unsigned int len)
{
	int timeout = 0;
    if(len==0)
		return 1;
    ...
    while(!I2C_CheckEvent(self->pp.I2Cx, I2C_EVENT_MASTER_MODE_SELECT));/*EV5,主模式*/
    ...
    while (!I2C_CheckEvent(self->pp.I2Cx, I2C_EVENT_MASTER_BYTE_TRANSMITTED));/*数据已发送*/
    ...
    while (len)
    {
		if(len==1)
		{
     		I2C_AcknowledgeConfig(self->pp.I2Cx, DISABLE);	/* 最后一位后要关闭应答的 */
    		I2C_GenerateSTOP(self->pp.I2Cx, ENABLE);			/* 发送停止位 */
		}
	    timeout = I2C_WaiteAckTime;
        do{
            if (timeout-- < 1)
...
static int I2c_Write(const struct I2c *self, unsigned char slaveAddr, unsigned char addr, unsigned char *buf, unsigned int len)
{
    int timeout = 0;
    int i = 0;
...
static int I2c_Read_16(const struct I2c *self, unsigned char slaveAddr, unsigned char addr, unsigned char *buf, unsigned int len)
{
	int timeout = 0;
    if(0 == len)
		return 1;
...
static int I2c_Write_16(const struct I2c *self, unsigned char slaveAddr, unsigned char addr, unsigned char *buf, unsigned int len)
{
    int timeout = 0;
    int i = 0;
...
void I2c_ReOpen(const struct I2c *self)
{
	...
	mencpy(self, newI2c, sizeof(struct I2c));
	free(newI2c);
}

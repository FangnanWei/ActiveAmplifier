...
void USART1_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)
	{
		...
		WriteComFifo(&g_ComFifo, g_Usart1RevBuf, temp);
		DMA_SetCurrDataCounter(DMA1_Channel5, USART_REV_BUF_SIZE);
		DMA_Cmd(DMA1_Channel5, ENABLE);
	}
}

static int UartConfigCheck(DI_UartConfig_t cfg)
{
	int i = 0;
	
	for (i = 0; i < DIM(BAUD_RATE_TABLE); i++){
		if (cfg.baudrate == BAUD_RATE_TABLE[i])
		{
			break;
		}
	}

	if (i == DIM(BAUD_RATE_TABLE)){
		return DI_IllegalArg;
	}

	if (cfg.type != MCU_UART_RS232){
		return DI_IllegalArg;
	}

	if (cfg.check != MCU_UART_CHECK_None &&
		cfg.check != MCU_UART_CHECK_EVEN &&
		cfg.check != MCU_UART_CHECK_ODD)
	{
		return DI_IllegalArg;
	}
...
int Usart2Cfg(DI_UartConfig_t cfg)
{
	...
	switch(cfg.check)
	{
		case MCU_UART_CHECK_None: USART_InitStructure.USART_Parity = USART_Parity_No; break;
		case MCU_UART_CHECK_ODD: USART_InitStructure.USART_Parity = USART_Parity_Odd; break;
		case MCU_UART_CHECK_EVEN: USART_InitStructure.USART_Parity = USART_Parity_Even; break;
	}
...
void USART2_IRQHandler(void)  
{
    ...
    if (USART_GetITStatus(USART2, USART_IT_IDLE) != RESET)
    {
        ...
        WriteComFifo(&g_ComFifo_2, g_Usart2RevBuf, temp);
...
{
	return ReadComFifo(&g_ComFifo, recvBuf, length);
}

unsigned short Usart2RecvData(unsigned char *recvBuf, unsigned short length)
{
    return ReadComFifo(&g_ComFifo_2, recvBuf, length);
}

...
int BeginUpgrade(ComFifo_t * pComFifo)
{
	...
	while(1)
	{
		...
		{
			if (SysTime::GetMs() - beginTime > UPGRADE_TIMEOUT)
            {
                SoftReset();
            }
		}
		...
		if (UPGRADE_RecvData == state){
			beginTime = SysTime::GetMs();		/* 更新时间，不至于超时 */
